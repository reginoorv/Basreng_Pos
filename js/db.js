// ===== BASRENG POS ONLINE — DB + UTILS =====
if(typeof SUPABASE_URL==='undefined'||SUPABASE_URL.includes('XXXX')){
  document.body.innerHTML=`<div style="font-family:sans-serif;max-width:480px;margin:60px auto;padding:24px;background:#fcebeb;border-radius:12px;color:#791f1f"><h2 style="margin-bottom:10px">⚠️ Konfigurasi Belum Diisi</h2><p>Buka file <code>js/config.js</code> dan isi <b>SUPABASE_URL</b> dan <b>SUPABASE_ANON</b> dengan data dari project Supabase kamu.</p><p style="margin-top:8px">Lihat <b>PANDUAN-DEPLOY.md</b> untuk langkah lengkapnya.</p></div>`;
  throw new Error('Config belum diisi');
}
...
async function dbUpdate(table,id,data){
  const{data:r,error}=await sb.from(table).update(data).eq('id',id).select().single();
  if(error) throw error;
  return r;
}
...
// AUTH
function hashPass(p){let h=0;for(let i=0;i<p.length;i++)h=Math.imul(31,h)+p.charCodeAt(i)|0;return'h'+Math.abs(h).toString(36)+p.length;}
function getSession(){try{return JSON.parse(sessionStorage.getItem('pos_session'));}catch{return null;}}
function setSession(u){sessionStorage.setItem('pos_session',JSON.stringify({id:u.id,username:u.username,nama:u.nama,role:u.role}));}
...
async function login(username,password){
  ...
  if(!user.aktif) throw new Error('Akun tidak aktif');
  setSession(user);
  return user;
}
...
async function kurangiStok(items,kode,actor){
  for(const item of items){
    ...
    await dbAdd('stok_log',{produk_id:p.id,nama_produk:item.nama,perubahan:-item.qty,stok_sebelum:p.stok,stok_sesudah:baru,keterangan:'Penjualan #'+kode,actor,waktu:new Date().toISOString()});
  }
}
async function kembalikanStok(items,kode,actor){
  for(const item of items){
    ...
    await dbUpdate('produk',p.id,{stok:baru,updated_at:new Date().toISOString()});
    await dbAdd('stok_log',{produk_id:p.id,nama_produk:item.nama,perubahan:+item.qty,stok_sebelum:p.stok,stok_sesudah:baru,keterangan:'Batal #'+kode,actor,waktu:new Date().toISOString()});
  }
}
...
async function setSetting(key,value){await dbUpsert('settings',{key,value});}

// UTILS
function generateKode(){const d=new Date();return`BSR-${String(d.getFullYear()).slice(2)}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}-${Math.floor(Math.random()*9000)+1000}`;}
...
function fmtDateShort(iso){if(!iso)return'-';const d=new Date(iso);return d.toLocaleDateString('id-ID',{day:'2-digit',month:'2-digit',year:'2-digit'})+' '+d.toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'});}
function showToast(msg,type=''){const t=document.getElementById('toast');if(!t)return;t.textContent=msg;t.className='show '+type;clearTimeout(t._t);t._t=setTimeout(()=>t.className='',3200);}
function tipePill(tipe){const m={agen:'pill-agen',ecer:'pill-ecer',mix:'pill-mix'};const l={agen:'Agen',ecer:'Ecer',mix:'Agen+Ecer'};return`<span class="pill ${m[tipe]||''}">${l[tipe]||tipe}</span>`;}
function statusPill(s){const m={pending:'pill-pending',lunas:'pill-lunas',batal:'pill-batal'};const l={pending:'Pending',lunas:'Lunas',batal:'Batal'};return`<span class="pill ${m[s]||''}">${l[s]||s}</span>`;}

// ---- PRINT dengan QR dan tunggu gambar load ----
function waitImgLoad(img){
  return new Promise(res=>{
    if(img.complete&&img.naturalWidth>0){res();return;}
    img.onload=res;
    img.onerror=res; // tetap lanjut walau gagal
    setTimeout(res,3000); // timeout 3 detik
  });
}
...
async function generatePrintHTML(t){
  ...
  // QR code: encode kode struk
  const qrUrl=`https://api.qrserver.com/v1/create-qr-code/?size=120x120&ecc=M&data=${encodeURIComponent(t.kode)}`;
  return`<div id="struk-wrap" style="font-family:'Courier New',monospace;width:72mm;padding:4mm 5mm;font-size:12px;line-height:1.55;background:#fff;color:#111">
    ...
    <div style="text-align:center;margin:6px 0">
      <img id="qr-img" src="${qrUrl}" width="90" height="90" crossorigin="anonymous" style="display:block;margin:0 auto;border:1px solid #eee"/>
      <div style="font-size:10px;color:#666;margin-top:3px">Scan untuk verifikasi</div>
    </div>
    <div style="border-top:1px solid #333;margin:6px 0"></div>
...
// Print dengan tunggu QR load dulu
async function doPrint(t){
  const pa=document.getElementById('print-area');
  pa.innerHTML=await generatePrintHTML(t);
  pa.style.display='block';
  // Tunggu gambar QR load
  const qrImg=pa.querySelector('#qr-img');
  if(qrImg){
    await waitImgLoad(qrImg);
    await new Promise(r=>setTimeout(r,300)); // buffer kecil
  }
  window.print();
...
function exportCSV(data,filename){
  ...
  data.forEach(row=>{rows.push(keys.map(k=>{let v=row[k]??'';if(typeof v==='string'&&(v.includes(',')||v.includes('"')))v='"'+v.replace(/"/g,'""')+'"';return v;}).join(','));});
...
async function exportExcel(transaksi,filename){
  ...
  const rows=transaksi.map(t=>({'Kode':t.kode,'Tipe':{agen:'Agen',ecer:'Ecer',mix:'Agen+Ecer'}[t.tipe]||t.tipe,'Pembeli':t.nama_pembeli||'-','Telepon':t.telp||'-','Items':t.items.map(i=>i.nama+' x'+i.qty).join('; '),'Subtotal':t.subtotal||t.total,'Diskon':t.diskon||0,'Total':t.total,'Status':{pending:'Pending',lunas:'Lunas',batal:'Batal'}[t.status]||t.status,'Admin':t.created_by||'-','Kasir':t.processed_by||'-','Tgl Buat':fmtDate(t.created_at),'Tgl Lunas':t.status==='lunas'?fmtDate(t.updated_at):'-'}));
